...
function StudentDBCreation2() {
  ...
  async function addXmlDataToDB() {
    let rollList=[]
    for(let i=1;i<xmlData.length;i++){
      let student= {};
      const rollNo=xmlData[i][0]
      rollList.push(rollNo)
      student["assignedBatch"]=batchName
      student["firstName"]=xmlData[i][1]
      student["lastName"]=xmlData[i][2]
      student["email"]=xmlData[i][3]
      student["contact"]=xmlData[i][4]
      student["role"]=xmlData[i][5]
    ...
    const batchId=(batchName.trim().toLowerCase()+year.trim()).replace(' ','')
  ...
  return (
    <div className="w-full h-full flex justify-center mt-5 md:mt-0 p-4 md:p-0 md:items-center">
      {modal && <Modal navigate={navigate} cancleEvent={closeModal} length={xmlData.length-1} batchName={batchName}/>}
...
function Modal( {navigate,cancleEvent , length, batchName}) {
  return (
    ...
    <div className="  animate-slide-in bg-green-100 border-green-900 rounded-xl shadow-xl p-4"
    style={{
      boxShadow: "rgba(255, 255, 255, 0.1) 0px 1px 1px 0px inset, rgba(50, 50, 93, 0.25) 0px 50px 100px -20px, rgba(0, 0, 0, 0.3) 0px 30px 60px -30px",
    }}
    >
      <div className=" text-center p-3 flex-auto justify-center ">
          <ThumbUpAltOutlined className='text-bold text-green-500'
            style={{ fontSize: "95px" }}
          
          />
        <h2 className="text-xl font-bold py-4 text-green-800">Batch added successfully!</h2>
